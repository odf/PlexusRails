...
describe "A Hash" do
  it "with two items the first of which is removed should not be empty" do
    @hash = Pazy::HashMap.with('A', true).with('B', true).without('A')
    @hash.should_not be_empty
  end

  context "when empty" do
    before :all do
      @hash = Pazy::HashMap.new
    end
  ...
  context "containing one item" do
    before :all do
      @hash = Pazy::HashMap.with("first", 1)
    end
  ...
  context "containing two items with different hash values" do
    before :all do
      ...
      @hash = Pazy::HashMap.with(@key_a, "a").with(@key_b, "b")
    end
  ...
  context "containing three items with identical hash values" do
    before :all do
      ...
      @hash = Pazy::HashMap.with(@key_a, "a").with(@key_b, "b").with(@key_c, "c")
    end
  ...
  context "containing a wild mix of items" do
    before :all do
      ...
      @hash = Pazy::HashMap.new + @keys.map { |key| [key, key.value] }
    end
  ...
  context "containing lots of items" do
    before :all do
      ...
      @hash = @keys.inject(Pazy::HashMap.new) { |h, key| h.with(key, key.value) }
    end
