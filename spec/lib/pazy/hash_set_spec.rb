...
describe "A Hash" do
  it "with two items the first of which is removed should not be empty" do
    @hash = Pazy::HashSet.with('A').with('B').without('A')
    @hash.should_not be_empty
  end

  context "when empty" do
    before :all do
      @hash = Pazy::HashSet.new
    end
  ...
  context "containing two items with different hash values" do
    before :all do
      ...
      @hash = Pazy::HashSet.with(@key_a).with(@key_b)
    end
  ...
  context "containing three items with identical hash values" do
    before :all do
      ...
      @hash = Pazy::HashSet.with(@key_a).with(@key_b).with(@key_c)
    end
